...
class MultiProcessSafeTimedRotatingFileHandler(TimedRotatingFileHandler):
    """
    该类是对 TimedRotatingFileHandler 的重写，解决其多进程不安全，丢失日志的问题

    TimedRotatingFileHandler 文件切割的逻辑为:

    1. 判断准备切入的文件 error.log.2024-02-28 是否存在，如果存在，则删除该文件
    2. 然后把 error.log 重命名为 error.log.2024-02-28
